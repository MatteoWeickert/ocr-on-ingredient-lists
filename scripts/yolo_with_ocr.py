...
class ImageAnalyzer:
    ...
    def process_cv2_picture(self, img_bgr):
        """
        ...
        gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
        # if np.std(gray) < 15:
        #     print("Wenig Kontrast im Graustufenbild erkannt. Wechsle zur Farbkanal-Analyse.")
        #     # Zerlege das Bild in seine BGR-Kanäle
        #     b, g, r = cv2.split(img_bgr)
            
        #     # Für rot/rosa-Fälle ist der Blau-Kanal ideal, da Rot keinen Blauanteil hat.
        #     # Der rote Text wird im b-Kanal dunkel, der rosa Hintergrund heller.
        #     # Wir invertieren den Blau-Kanal, damit der Text schwarz wird.
        #     gray = cv2.bitwise_not(b)
    ...
    def normalize_ingredients(self, raw_text: str) -> str:
        ...
        text = self.join_hyphenated_lines(text)
        ...
        text = self.cut_to_ingredients(text)
        ...
        filtered_words = merged_words

        text = " ".join(filtered_words)  # Füge die gefilterten Wörter wieder zusammen
             
        text = re.sub(r"\s+", ' ', text).strip().lower()

        text = self.add_lost_commata(text)  # Füge verlorene Kommas hinzu
    ...
    def normalize_nutrition(self, word: str) -> str:
        ...
        sub_rules = {
            ...
            r'(?<=[A-Za-zÄÖÜäöüß])\.(?=[A-Za-zÄÖÜäöüß])': ' ', # Ersetze Punkte zwischen Buchstaben durch Leerzeichen
            r':': " ",  # Entferne Doppelpunkte

            r'\.\.': '.',           # Ersetzt ".." mit "."



            r'([0-9])o': r'\1g', # 10 o -> 10g
            r'([0-9])l': r'\1g',
            r'([;])': '.', # Ersetze Semikolons mit .
            r'\s{2,}': ' ', # Entferne überflüssige Leerzeichen
            r'\s*,\s*': '.', # Entferne Leerzeichen vor und nach Kommas

            r'(^|\s)-(?=\s*[a-zA-Z])': r'\1',  # Entferne Bindestrich am Wortanfang vor Buchstaben oder Leerzeichen+Buchstabe

            r'["\'()„“‚‘`´❝❞°]': ''  # Entferne Anführungszeichen und weitere Sonderzeichen

        }

        print(f"Vor der Normalisierung nach sub_rules: {word}")
        ...
        word = re.sub(
            r'(?i)\b(?:ca\.?|approx\.?|~)?\s*<?\s*'         # optionale Präfixe + optionales '<'
            r'(\d+(?:[.,]\d+)*(?:/\d+(?:[.,]\d+)*)?)'       # Zahl (inkl. Dezimal/Bruch)
            r'\s*'                                          # evtl. Leerraum
            r'(kj|kcal|g|mg|µg|ug|ml|l|%)\b',               # Einheit
            r'\1\2',
            word
        )
        ...
        for word in corrected_words:
            ...
            for part in parts:
                ...
                elif len(part_without_parentheses) >= 3 and part_without_parentheses.isalpha():
    ...
    def compute_clusters_by_DBSCAN(self, processed_rows):
        """
    ...
    def merge_clusters_semantically(self, centers, processed_rows):
        """
                ...
                if word1 in SEMANTIC_PAIRS and i < len(row) - 1:
                    ...
                    if word2 == SEMANTIC_PAIRS[word1]:
                        ...
                        if cid1 != cid2:
                            ...
                            return self.merge_clusters_semantically(merged_centers, processed_rows)  # Rekursiv aufrufen, um weitere Zusammenführungen zu prüfen
                        
        return merged_centers  # Rückgabe der finalen Zentren nach semantischem Merging

    def compute_clusters_by_x(self, x_values):
        """
    ...
    def process_calculated_clusters(self, processed_rows, min_row_coverage_ratio=0.2):
        """
        ...
        # centers = self.compute_clusters_by_x(x_values)
        centers = self.compute_clusters_by_DBSCAN(processed_rows)  # Alternativ: DBSCAN verwenden
        #centers = self.merge_clusters_semantically(raw_centers, processed_rows)  # Semantisches Merging der Cluster
    ...
    def detect_label_cluster(self, processed_rows, centers):
        """
        ...
        for cid, center in enumerate(centers):
            ...
            if not tokens:
                ...
                number_ratio = sum(1 for token in tokens if self.is_numeric(token)) / len(tokens)  # Anteil der Zahlen in den Tokens
    ...
    def find_body_start_index(self, processed_rows):
        """
    ...
    def find_footer_start_index(self, processed_rows):
        """
    ...
    def is_number_or_unit(self, tok: str) -> bool:
        t = tok.lower()
    ...
    def row_metrics(self, row, centers):
        # Breiten/Koordinaten
        ...
        numeric_ratio = (sum(1 for t in texts if self.is_number_or_unit(t)) / len(texts)) if texts else 0.0
    ...
    def derive_title_and_column_names(self, header_rows, centers, label_cluster):
        """
        ...
        metrics1 = self.row_metrics(first_row, centers)

        # Prüfe Abstand zunächster Zeile

        big_vertical_gap = False
        if len(header_rows) > 1:
            metrics2 = self.row_metrics(header_rows[1], centers)
    ...
    def is_empty_value_row(self, row, label_idx):
        """
    ...
    def merge_davon_rows(self, rows_text, rows_boxes, label_cluster):
        ...
        while i < len(rows_text):
            ...
            if label_row.startswith("davon") and label_row != "davon zucker" and self.is_empty_value_row(current_row_text, label_cluster) and (i + 1 < len(rows_text)):
    ...
    def merge_multiline_energy_rows(self, rows_text, rows_boxes, label_cluster, centers):
        """
    ...
    def join_hyphenated_lines(self, text: str) -> str:
    ...
    def cut_to_ingredients(self, text:str) -> str:
        """
    ...
    def add_lost_commata(self, text: str) -> str:
        """
    ...
    def merge_num_unit_in_row(self, row):
        """
        ...
        while i < len(row):
            ...
            if i + 1 < len(row):
                ...
                current_core = current_word[4].strip().strip("()[]{}")
    ...
    def main(self, image_paths, target_class):
        ...
        self.create_picture_preview(preprocessed, self.croppedImg, size=(640, 480))
        ...
        if target_class == "ingredients":
            ...
            for i, txt in enumerate(data["text"]):
                ...
                boxes.append((x1, y1, x1 + w, y1 + h, token))

            self.draw_bboxes(preprocessed, boxes)
            
            # Postprocessing OCR-Ergebnisse

            text = self.normalize_ingredients(text)
            
            print(f"Normalized Text: {text.strip()}")

            output = {"ingredients": text.strip()}
            ...
            for i, txt in enumerate(data["text"]):
                ...
                normalized_txt = self.normalize_nutrition(txt)
            ...
            for row in rows: # row ist ein List[Box]
                ...
                row = self.merge_num_unit_in_row(row)  # Mergen von Einheiten in der Zeile
                ...
                for i, box in enumerate(row):
                    ...
                    # HEURISTIK 2: "289g" -> "28g" (Fehler: Leerzeichen als '9' gelesen)
                    ...
                    if match and is_salt_line == False:
                        ...
                        print(f"Kontext-Korrektur (Heuristik 2c): '{word}' -> '{corrected_word}'")
                        new_box = (box[0], box[1], box[2], box[3], corrected_word)  # Ersetze das Wort in der Box
                        corrected_boxes.append(new_box)
                        continue
            ...
            body_start_index = self.find_body_start_index(final_processed_rows)
            footer_start_index = self.find_footer_start_index(final_processed_rows)
            ...
            centers = self.process_calculated_clusters(body_rows, min_row_coverage_ratio=0.2)
            ...
            label_cluster = self.detect_label_cluster(rows_compact_boxes, centers_filtered)

            # Merge "davon" Zeilen, falls Label-Spalte gefunden wurde
            rows_compact_text, rows_compact_boxes = self.merge_davon_rows(rows_compact_text, rows_compact_boxes, label_cluster)

            # Merge zweizeilige Energie-Zeilen
            rows_compact_text, rows_compact_boxes = self.merge_multiline_energy_rows(rows_compact_text, rows_compact_boxes, label_cluster, centers_filtered)

            # Ermittle Titel, Spaltennamen und Fußnote
            title, column_names = self.derive_title_and_column_names(header_rows, centers_filtered, label_cluster)
            ...
            for i, row in enumerate(rows_output, 1):
                print(f"{i:02d}: {row['label']} | {' | '.join(row['values'])}")
            print("Fußnote:", footer)
